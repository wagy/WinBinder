...
function wb_get_text($ctrl, $item=null, $subitem=null, $readSelected=false)
{
	if(!$ctrl)
		return null;
	...
	if(wb_get_class($ctrl) == ListView) {
		...
		if($item !== null) {		// Valid item
			...
			$sel = ($readSelected ? null : wb_get_selected($ctrl));
...
function wb_add_text($ctrl, $text, $item=null, $subitem=null)
{
...
function wb_sys_dlg_open($parent=null, $title=null, $filter=null, $path=null, $filename=null, $flags = null, $chDir=true)
{
	...
	if ($chDir)
	{
		//Achtung wbtemp_sys_dlg_open verändert das aktuelle Verzeichnis
...
/**
 * Set listeview colors
 *
 * @param int $lparam3 Address (send by listview color handler)
 *                     of color struct  {mode:0, foreground:0, background:0} C style
 * @param int $fore    0xBBGGRR [optional]
 * @param int $back    0xBBGGRR [optional]
 * @return int WBC_LV_DEFAULT|WBC_LV_DRAW     Set new color
 */
function wb_set_listview_color($lparam3=0, $fore=NOCOLOR, $back=NOCOLOR)
{
    $ret = WBC_LV_DEFAULT;
	if ($lparam3 > 0)
	{
		...
		if ($mode != WBC_LV_NONE)
		{
            wb_poke($lparam3, pack("iLL", $mode, $setfore, $setback), 12);
            $ret = WBC_LV_DRAW;
		}
	}
    return $ret;
